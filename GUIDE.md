...
export default {
    ...
    mounted() {
        // 개별 파라미터 (기본값 지정 가능)
        this.userId = this.getParam('id', 0);      // id가 없으면 0
        this.tab = this.getParam('tab', 'profile'); // tab이 없으면 'profile'
...
> **중요**: `getParam()`을 사용하는 것이 권장됩니다. `this.$query`나 `this.$params`는 `data()` 함수 내에서 사용할 수 없으므로 피하세요.
